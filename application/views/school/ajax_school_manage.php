...
<script>
	...
	var tbl_usr_info = '<?=base_url()?>' + 'school/search_lession_table';

	function createSchoolModal() {
		add_school_view_modal = $("#add_school_view_modal");
		let settings = {
			"url": base_url + 'school/ajax_create_school/',
			"method": "get"
		};
		$('#add_school_view_modal').iziModal({
			...
			onOpening: function (modal) {
				...
				$.ajax(settings).done(function (response) {
					...
					$("#add_school_view_modal .iziModal-content").html(response);
					modal.stopLoading();
				});
			}
		});
	...
	function addSchoolProc() {
		$('#add_school_from_id').validate({

			rules: {
                lession_title: { required: !0, },
                lession_doctor_name: {required: !0, },
				start_time: { required: !0, },
				lession_password: { required: !0, },
            },
			messages: {
                lession_title: {required: "教学主题请输入" },
                lession_doctor_name: { required: "教学导师请输入" },
                start_time: { required: "开始时间设置它" },
				lession_password: { required: "密码请输入" },
            },
			errorPlacement: function (error, element) {
				var placement = $(element).data('error');
				if (placement) {
					$(placement).append(error);
					$(".form-group .error").css("margin-top", "4em");
			...
			submitHandler: function (e) {
				add_School_proc_submit();
			}
		});
		$('#add_school_from_id').submit();


	}

	function add_School_proc_submit(){
