						...
						<div class="portlet portlet-sortable box blue-madison portlet-datatable ">
							...
							<div class="portlet-body">
								<div class="clearfix" style="margin-left: 10%;">
									...
									<button type="button" class="btn blue" id="report_detail_view" disabled onclick="report_detail_view()">
									...
									<button type="button" id="dicom_view_btn" disabled class="btn green" onclick="dicom_view()">
									...
									<?php endif; ?>
									<?php if ($usrRole == 1||$usrRole == 100||$usrRole == 10||$usrRole == 1024||$this->session->userdata('IntelligentDiagnosis')):?>
									<button type="button" disabled class="btn  red-mint" id="contact_proc" onclick="">
										<span class="glyphicon glyphicon-share"> </span>智能诊断</button>
									<?php endif; ?>
									<?php if ($usrRole == 1||$usrRole == 100||$usrRole == 10||$usrRole == 1024||$this->session->userdata('RemoteDiagnosis')):?>
									<button type="button" disabled class="btn  yellow-mint" id="remote_contact_proc_btn" onclick="remote_contact_proc()">
										<span class="glyphicon glyphicon-share"> </span>远程诊断</button>
									<?php endif; ?>
								</div>
								<table class="table table-striped table-bordered table-hover" id="report_table_info">
									<thead>
										<tr>
											<th>序号 </th>
											<th>病人编号</th>
						...
						<div class="portlet portlet-sortable box blue-madison">
							...
							<div class="portlet-body">
								<div class="scroller" style="height:200px" data-rail-visible="1" data-rail-color="yellow" data-handle-color="#a1b2bd">
									<strong>
										<?=$this->lang->line('image_expression')?>:</strong>
									<br/>
									<span id="Imaging_performance"></span>
								</div>
							</div>
						</div>
						<!-- END Portlet PORTLET-->
					...
					<div class="col-md-3">
						...
						<div class="portlet portlet-sortable box blue-madison">
							...
							<div class="portlet-body">
								<div class="scroller" style="height:200px" data-rail-visible="1" data-rail-color="yellow" data-handle-color="#a1b2bd">
									<strong>
										<?=$this->lang->line('impression')?>:</strong>
									<br/>
	...
	<script>
		...
		function past_checkup_info(val) {
			...
			if (booking_status == '1') {
				...
				$("#contact_proc").attr("disabled", "true");
				$("#remote_contact_proc_btn").attr("disabled", "true");
				...
				$('#remote_contact_proc_btn').removeAttr('disabled');
				...
				$('#report_btn').attr('disabled', true).html('<span class="glyphicon glyphicon-send"> </span>诊断报告');
				$("#report_detail_view").attr("disabled", "true");
				$('#dicom_view_btn').removeAttr('disabled');
				$('#remote_contact_proc_btn').removeAttr('disabled');
			}
			var remote_status = val.remote_status;
			if (remote_status == '1') {
			...
			$.post(strURL).done(function (data) {
				...
				if (report_data) {
					...
					for (var i = 0; i < count; i++) {
						var patient_type = '';
						if (report_data.tdata[i].patient_type == 1) {
							patient_type = '急诊';
						} else if (report_data.tdata[i].patient_type == 2) {
							patient_type = 'VIP';
						} else {
							patient_type = '平诊';
						}
						...
						if (report_data.tdata[i].patient_gender == '1') {
							var gender =
								'<button type="button" class="btn btn-circle btn-icon-only red"><span aria-hidden="true" class="icon-symbol-female"></span></button>';
						} else {
							var gender =
								'<button type="button" class="btn btn-circle btn-icon-only blue"><span aria-hidden="true" class="icon-symbol-male"></span></button>';
						}
						display_data +=
							'<tr><td><label class="mt-radio mt-radio-single mt-radio-outline"><input type="radio" name=selected_past_report class="md-radiobtn" onchange="selected_past_report(this.value)" value="' +
							report_data.tdata[i].report_id + '" /><span></span></label></td>';
						display_data += "<td>" + report_data.tdata[i].patient_code + "</td>" + "<td>" + report_data.tdata[i].image_num +
							"</td><td>" + report_data
							.tdata[i].patient_name + "</td>" + "<td>" + gender +
							"</td>" + "<td>" + report_data.tdata[i].patient_age +
							"</td>" + "<td>" + patient_type + "</td>";
						display_data += "<td>" + report_data.tdata[i].report_doc_name + "</td>" +
		...
		function search_clear() {
			$('#search_report_form')[0].reset();
		};

		$(document).ready(function () {
			var tbl_usr_info = '<?=base_url()?>' + 'report/search_data';
			...
			$('#report_table_info tbody ').on('click', 'tr', function (){
				...
				$.ajax({
					...
					success: function (response) {
						...
						// var booking_id = $("#booking_id").val(response.delivery_name).val();
						// var strURL = base_url + "report/reporting/" + booking_id;
						// window.location.href = strURL;
					}
				});
			...
			$("#search_report_btn").click(function () {
				...
				var ajaxurl = base_url + 'report/search_data?' + formData;
