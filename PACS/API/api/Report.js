...
export default class Reporting {
	...
	reportList(request, response) {
		...
		if (request.query.hasOwnProperty('checkup_status')) query.booking_status = request.query.checkup_status;
		if (request.query.hasOwnProperty('checkup_type')) query.checkup_type = request.query.checkup_type;
				...
				for (var key in data) {
					if (data.hasOwnProperty(key)) {
						...
						ReportData[index] = {
							...
							checkup_status: data[key].checkup_status,
		...
		this.ReportingModel.SaveReportInfo(saveData, where)
			.then((data) => {
				if (data == 0) {
					...
					let status = request.query.hasOwnProperty('save') ? 7 : 4
					let updateCheckQuery = {
						chc_booking_id: saveData.booking_id
					};
					let updatecheckupData = {
						checkup_status: status,
					};
					this.ReportingModel.UpdateCheckuoData(updatecheckupData, updateCheckQuery).then((data) => {
						if (data != 0) {
							return response.json({
								status: "success"
							});
	...
	myReportList(request, response) {
		...
		if (request.query.hasOwnProperty('checkup_status')) query.checkup_status = request.query.checkup_status;
	...
	deliberation(request, response) {
		...
		if (request.query.hasOwnProperty('checkup_status')) checkup_status = request.query.checkup_status;
		if (where.deliberation_id) {
	...
	getTemplate(request, response) {
		this.ReportingModel.getTemplateData().then(data => {
			if (!data.length) {
				return response.send({
					data: null,
					message: 'There is no template Data!',
					response_code: 0
				});
			}
			for (const key in data) {
				send_data[key] = {
					device: data[key].report_class,
					class: {
						class_name: data[key].class_name,
							class_id: data[key].class_id,
							subclass: {
								subclass_name: data[key].module_name,
								subclass_id: data[key].module_id,
								template: {
									template_image: data[key].image_expression,
									template_id: data[key].template_id,
									template_name: data[key].template_name,
									template_suggestion: data[key].recommended_report
								}
							}
					}
				};
	...
	async getTemplateDeviceAndClass(request, response) {
		let module_class = await this.ReportingModel.getModuleClass();
		let send_data = {};
		this.ReportingModel.getDeviceType().then(data => {
			send_data = {
				template_class: module_class,
				template_device: data
			}
			return response.send({
				data: send_data,
	...
	getModuleClass(request, response) {
		let query = {};
		if (request.query.hasOwnProperty('template_device')) query.report_class = request.query.template_device;
		if (request.query.hasOwnProperty('template_class_id')) query.module_class = request.query.template_class_id;
		this.ReportingModel.getModule(query).then(data => {
			return response.send({
				data: data,
				message: 'successfully Get!',
				response_code: 1
	...
	getTemplateContent(request, response) {

		let query = {};
		if (request.query.hasOwnProperty('template_content_id')) query.template_id = request.query.template_content_id;
		if (request.query.hasOwnProperty('template_device')) query.device_type = request.query.template_device;
		if (request.query.hasOwnProperty('template_subclass_id')) query.report_module_id = request.query.template_subclass_id;
		console.log(query);
		this.ReportingModel.getTemplateData(query).then(data => {
			return response.send({
				data: data,
				message: 'successfully Get!',
				response_code: 1
	...
	saveTemplate(request, response) {
		...
		let saveData = {
			template_name: request.query.template_title,
			device_type: request.query.template_device,
			report_module_id: request.query.template_subclass_id,
			recommended_report: request.query.template_content_suggestion,
			image_expression: request.query.template_content_image,
			checkup_name: request.query.template_content_checkup,
			create_time: moment().format('YYYY-MM-DD h:mm:ss')
		};
		let where;
		if (request.query.hasOwnProperty('template_id')) {
			where = {
				template_id: request.query.template_id
			};
		}
		console.log(saveData, 'where is ', where);
		this.ReportingModel.SaveTemplate(saveData, where).then(data => {
			return response.send({
				data: data,
				message: 'successfully save Template data!',
				response_code: 1
			});
			...
			return response.send({
				data: err,
				message: 'can not save template data',
				response_code: 0
			});
		});
	}
	...
	saveTemplateClass(request, response) {

		let saveData = {
			module_name: request.query.template_subclass_name,
			module_class: request.query.template_class_id,
			report_class: request.query.template_device,
			create_time: moment().format('YYYY-MM-DD h:mm:ss')
		};
		let where;
		if (request.query.hasOwnProperty('module_id')) {
			where = {
				module_id: request.query.template_subclass_id
			};
		}
		this.ReportingModel.saveTemplateClass(saveData, where).then(data => {
			return response.send({
				data: data,
				message: 'successfully save Template sub class data!',
				response_code: 1
			});
			...
			return response.send({
				data: err,
				message: 'can not save template sub class data',
				response_code: 0
			});
		});
	}

	removeTemplateClass(request, response) {
		let where = {
			module_id: request.query.template_subclass_id
		};
		this.ReportingModel.removeTemplateClass(where).then(data => {
			return response.send({
				data: data,
				message: 'successfully remove Template sub class!',
				response_code: 1
			});
		}).catch(err => {
			return response.send({
				data: err,
				message: 'Can not remove template sub class data',
				response_code: 0
			});
	...
	removeTemplate(request, response) {
		let where = {
			template_id: request.query.template_content_id
		};
		this.ReportingModel.deleteTemplate(where).then(data => {
			return response.send({
				data: data,
				message: 'successfully remove Template data!',
				response_code: 1
			});
		}).catch(err => {
			return response.send({
				data: err,
				message: 'Can not remove template data',
				response_code: 0
			});
