...
export default class Statistics {
	...
	getBookingData(req, res) {
		let data_type = req.query.date_type;
		...
		let date_part_month = req.query.hasOwnProperty('date_Part') ? req.query.date_Part : moment().format('YYYY-MM');
		let date_part_year = req.query.hasOwnProperty('date_Part') ? req.query.date_Part : moment().format('YYYY');
		...
		let source_type = req.query.hasOwnProperty('source_type') ? req.query.source_type : null;
		console.log(data_type,chartType, source_type, date_part )
		...
		switch (parseInt(chartType)) {
			case 0:
				...
				var queryarray = {};
				for (var index = 0; index < monthDate[month]; index++) {
					queryarray[index] = [date_part_month + '-0' + index + ' 00:00:00', date_part_month + '-0' + index + ' 23:59:59'];
			...
			case 1:
				var queryarray = {};
				for (var index = 1; index < 13; index++) {
					queryarray[index] = [date_part_year + '-' + months[index] + '-01 00:00:00', date_part_year + '-' + months[index] + '-' + monthDate[index] + ' 23:59:59'];
				}
				// console.log(queryarray);
				getBookingInfoByDate(realQuery, queryarray);
				break;
	...
	getReportData(request, response) {
		let data_type = request.query.date_type;
		...
		let date_part_month = request.query.hasOwnProperty('date_Part') ? request.query.date_Part : moment().format('YYYY-MM');
		let date_part_year = request.query.hasOwnProperty('date_Part') ? request.query.date_Part : moment().format('YYYY');
		...
		if(device_type == '1'){
			realQuery.checkup_type = 'CT';
		}
		else if(device_type == '2'){
			realQuery.checkup_type = 'DR';
		}
		else if(device_type == '3'){
			realQuery.checkup_type = 'MR';
		}
		else if(device_type == '4'){
			realQuery.checkup_type = 'UR';
		}
	...
	getRemoteData(request, response) {
		...
		var curtDate = date_part;
		let realQuery = this.getQueryRole();
		if (remote_type != '0') realQuery.contact_type = remote_type;
		switch (parseInt(chartType)) {
			case 0:
				var month = parseInt(curtDate.split('-')[1]);
				var queryarray = {};
