export default class BaseModel {
	...
	getTemplateData(query) {
		...
		if (query) {
			...
			templateData = this.DBCon('tbl_template')
				...
				.where(query)
				.select(
					"template_id as template_content_id",
					"template_name as template_content_title",
					"image_expression as template_content_image",
					"recommended_report as template_content_suggestion",
					"device_type",
					"checkup_name",
					"report_module_id as subclass_id"
				);
		} else {

			templateData = this.DBCon.select(
					"template_id as template_content_id",
					"template_name as ",
					"image_expression as template_content_image",
					"recommended_report as template_content_suggestion",
	...
	getModuleClass(query) {
		var modules = [];
		if (query) {
			modules = this.DBCon('tbl_module_class').where(query).select();
		} else {
	...
	getModule(query) {
		...
		if (query) {
			templateData = this.DBCon('tbl_report_module')
				.join('tbl_module_class', 'tbl_module_class.class_id', '=', 'tbl_report_module.module_class')
				.where(query).select(
					"module_name as template_subclass_name",
					"module_id as template_subclass_id"
				);
		} else {
			templateData = this.DBCon.select(
					"module_name as template_subclass_name",
					"module_id as template_subclass_id"
				).from('tbl_report_module')
				.join('tbl_module_class', 'tbl_module_class.class_id', '=', 'tbl_report_module.module_class');
	...
	GetDeliberationData(query) {
		...
		if (query) {
			...
			templateData = this.DBCon('tbl_patient_booking')
				.join('tbl_report', 'tbl_patient_booking.booking_id', '=', 'tbl_report.booking_id')
				.join('tbl_deliberation', 'tbl_deliberation.deli_rpt_id', '=', 'tbl_report.report_id')
				.where(query)
				.select()
		} else {
			templateData = this.DBCon.from('tbl_patient_booking')
	...
	getHospitalClass(query){
		let hosList = [];
		if (query) {
			hosList = this.DBCon.select('hospital_class').from('tbl_hospital').where(query);
		} else {
			hosList = this.DBCon.select('hospital_class').from('tbl_hospital');
		}
		return hosList;
