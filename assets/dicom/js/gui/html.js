 ...
 */
BAMBOO.html.appendCell = function(row, text)
{
 ...
 */
BAMBOO.html.appendHCell = function(row, text)
{
 ...
 * @param {} rowHeader
 */
BAMBOO.html.appendRowForArray = function(table, input, level, maxLevel, rowHeader)
{
    var row = null;
    // loop through
    for(var i=0; i<input.length; ++i) {
        ...
        if( typeof input[i] === 'number' ||
            ...
            BAMBOO.html.appendCell(row, input[i]);
        }
        // last level
        else {
            BAMBOO.html.appendRow(table, input[i], level+i, maxLevel, rowHeader);
        }
    }
};
 ...
 * @param {} rowHeader
 */
BAMBOO.html.appendRowForObject = function(table, input, level, maxLevel, rowHeader)
{	
    ...
    for( var o=0; o<keys.length; ++o ) {
        ...
        if( typeof input[keys[o]] === 'number' ||
            ...
            if( o === 0 && rowHeader) {
                BAMBOO.html.appendCell(row, rowHeader);
            }
            BAMBOO.html.appendCell(row, input[keys[o]]);
        }
        // last level
        else {
            BAMBOO.html.appendRow(table, input[keys[o]], level+o, maxLevel, keys[o]);
        }
    }
    // header row
    ...
    if( level === 2 ) {
        ...
        if( rowHeader ) {
            BAMBOO.html.appendHCell(th, "Name");
        }
        for( var k=0; k<keys.length; ++k ) {
            BAMBOO.html.appendHCell(th, BAMBOO.utils.capitaliseFirstLetter(keys[k]));
        }
    }
};
 ...
 * @param {} rowHeader
 */
BAMBOO.html.appendRow = function(table, input, level, maxLevel, rowHeader)
{
    // array
    if( input instanceof Array ) {
        BAMBOO.html.appendRowForArray(table, input, level+1, maxLevel, rowHeader);
    }
    // object
    else if( typeof input === 'object') {
        BAMBOO.html.appendRowForObject(table, input, level+1, maxLevel, rowHeader);
    }
    else {
 ...
 */
BAMBOO.html.toTable = function(input)
{
    var table = document.createElement('table');
    BAMBOO.html.appendRow(table, input, 0, 2);
    return table;
};
 ...
 */
BAMBOO.html.getHtmlSearchForm = function(htmlTableToSearch)
{
    var form = document.createElement("form");
    form.setAttribute("class", "filter");
    var input = document.createElement("input");
    input.onkeyup = function() {
        BAMBOO.html.filterTable(input, htmlTableToSearch);
    };
 ...
 */
BAMBOO.html.filterTable = function(term, table) {
    // de-highlight
    BAMBOO.html.dehighlight(table);
    // split search terms
    ...
    for (var r = 1; r < table.rows.length; ++r) {
        ...
        for (var i = 0; i < terms.length; ++i) {
            ...
            if (text.indexOf(terms[i]) < 0) {
                ...
                if (terms[i].length) {
                    BAMBOO.html.highlight(terms[i], table.rows[r]);
                }
            }
 ...
 */
BAMBOO.html.dehighlight = function(container) {
 ...
 */
BAMBOO.html.highlight = function(term, container) {
    for (var i = 0; i < container.childNodes.length; i++) {
        var node = container.childNodes[i];
        ...
        if (node.nodeType === 3) {
            ...
            if (data_low.indexOf(term) >= 0) {
                ...
                while ((result = data_low.indexOf(term)) !== -1) {
                    ...
                    // term
                    new_node.appendChild(BAMBOO.html.createHighlightNode(
            ...
            // Keep going onto other elements
            BAMBOO.html.highlight(term, node);
        }
    }
};
 ...
 */
BAMBOO.html.createHighlightNode = function(child) {
 ...
 */
BAMBOO.html.cleanNode = function(node) {
    // remove its children
 ...
 */
BAMBOO.html.removeNode = function(nodeId) {
    // find the node
 ...
 */
BAMBOO.html.createHtmlSelect = function(name, list) {
    // select
    ...
    if( list instanceof Array )
    {
        for ( var i in list )
        {
            ...
            option.value = list[i];
            option.appendChild(document.createTextNode(BAMBOO.utils.capitaliseFirstLetter(list[i])));
    ...
    else if( typeof list === 'object')
    {
        for ( var item in list )
        {
            option = document.createElement("option");
            option.value = item;
            option.appendChild(document.createTextNode(BAMBOO.utils.capitaliseFirstLetter(item)));
            select.appendChild(option);
        }
    }
    else
 ...
 *  [BAMBOO root]?input=encodeURI([root]?key0=value0&key1=value1)
 * or
 *  [BAMBOO root]?input=encodeURI([manifest link])&type=manifest
 *  
 * @method getUriParam
 * @static
 ...
 */
BAMBOO.html.getUriParam = function(uri)
{
    var inputUri = uri || window.location.href;
    // split key/value pairs
    var mainQueryPairs = BAMBOO.utils.splitQueryString(inputUri);
    // check pairs
    ...
    if( query.type && query.type === "manifest" ) {
        result = BAMBOO.html.decodeManifestUri( query.input, query.nslices );
    }
    // if key/value uri
    else {
        result = BAMBOO.html.decodeKeyValueUri( query.input, query.BAMBOOReplaceMode );
    }
 ...
 */
BAMBOO.html.decodeKeyValueUri = function(uri, replaceMode)
{
    var result = [];
    ...
    var inputQueryPairs = BAMBOO.utils.splitQueryString(queryUri);
 ...
 */
BAMBOO.html.decodeManifestUri = function(uri, nslices)
{
    var result = [];
    ...
    {
        result = BAMBOO.html.decodeManifest(this.responseXML, nslices);
    };
    ...
    //request.onprogress = BAMBOO.gui.updateProgress;
    request.send(null);
 ...
 * @param {Number} nslices The number of slices to load.
 */
BAMBOO.html.decodeManifest = function(manifest, nslices)
{
    var result = [];
    // wado url
 ...
 */
BAMBOO.html.displayElement = function (id, flag)
{
 ...
 * @param {Number} id The id of the element to toggle its display.
 */
BAMBOO.html.toggleDisplay = function (id)
{
    var element = document.getElementById(id);
    if ( element ) {
 ...
 */
BAMBOO.html.appendElement = function (parentId, element)
{
 ...
 */
BAMBOO.html.createHiddenElement = function (type, id)
{
