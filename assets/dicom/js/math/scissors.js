 ...
 * 
 */
BAMBOO.math.computeGreyscale = function(data, width, height) {
 ...
 * 
 */
BAMBOO.math.computeGradient = function(greyscale) {
 ...
 * 
 */
BAMBOO.math.computeLaplace = function(greyscale) {
...
BAMBOO.math.computeGradX = function(greyscale) {
...
BAMBOO.math.computeGradY = function(greyscale) {
...
BAMBOO.math.gradUnitVector = function(gradX, gradY, px, py, out) {
...
BAMBOO.math.gradDirection = function(gradX, gradY, px, py, qx, qy) {
    var __dgpuv = new BAMBOO.math.FastPoint2D(-1, -1); 
    var __gdquv = new BAMBOO.math.FastPoint2D(-1, -1);
    // Compute the gradiant direction, in radians, between to points
    BAMBOO.math.gradUnitVector(gradX, gradY, px, py, __dgpuv);
    BAMBOO.math.gradUnitVector(gradX, gradY, qx, qy, __gdquv);
...
BAMBOO.math.computeSides = function(dist, gradX, gradY, greyscale) {
    ...
    var guv = new BAMBOO.math.FastPoint2D(-1, -1); // Current gradient unit vector
    ...
    for ( var y = 0; y < gradX.length; y++ ) {
        ...
        for ( var x = 0; x < gradX[y].length; x++ ) {
            BAMBOO.math.gradUnitVector(gradX, gradY, x, y, guv);
...
BAMBOO.math.gaussianBlur = function(buffer, out) {
...
BAMBOO.math.Scissors.prototype.getTrainingIdx = function(granularity, value) {
    return Math.round((granularity - 1) * value);
};

BAMBOO.math.Scissors.prototype.getTrainedEdge = function(edge) {
    return this.edgeTraining[this.getTrainingIdx(this.edgeGran, edge)];
};

BAMBOO.math.Scissors.prototype.getTrainedGrad = function(grad) {
    return this.gradTraining[this.getTrainingIdx(this.gradGran, grad)];
};

BAMBOO.math.Scissors.prototype.getTrainedInside = function(inside) {
    return this.insideTraining[this.getTrainingIdx(this.insideGran, inside)];
};

BAMBOO.math.Scissors.prototype.getTrainedOutside = function(outside) {
    return this.outsideTraining[this.getTrainingIdx(this.outsideGran, outside)];
};
// End training methods //

BAMBOO.math.Scissors.prototype.setWorking = function(working) {
    // Sets working flag
    this.working = working;
};

BAMBOO.math.Scissors.prototype.setDimensions = function(width, height) {
    this.width = width;
    this.height = height;
};

BAMBOO.math.Scissors.prototype.setData = function(data) {
    if ( this.width === -1 || this.height === -1 ) {
        // The width and height should have already been set
        throw new Error("Dimensions have not been set.");
    }

    this.greyscale = BAMBOO.math.computeGreyscale(data, this.width, this.height);
    this.laplace = BAMBOO.math.computeLaplace(this.greyscale);
    this.gradient = BAMBOO.math.computeGradient(this.greyscale);
    this.gradX = BAMBOO.math.computeGradX(this.greyscale);
    this.gradY = BAMBOO.math.computeGradY(this.greyscale);
    
    var sides = BAMBOO.math.computeSides(this.edgeWidth, this.gradX, this.gradY, this.greyscale);
...
BAMBOO.math.Scissors.prototype.findTrainingPoints = function(p) {
...
BAMBOO.math.Scissors.prototype.resetTraining = function() {
    this.trained = false; // Training is ignored with this flag set
};

BAMBOO.math.Scissors.prototype.doTraining = function(p) {
...
BAMBOO.math.Scissors.prototype.calculateTraining = function(buffer, granularity, input, output) {
    var i = 0;
    ...
    BAMBOO.math.gaussianBlur(buffer, output);
};

BAMBOO.math.Scissors.prototype.addInStaticGrad = function(have, need) {
...
BAMBOO.math.Scissors.prototype.gradDirection = function(px, py, qx, qy) {
    return BAMBOO.math.gradDirection(this.gradX, this.gradY, px, py, qx, qy);
};

BAMBOO.math.Scissors.prototype.dist = function(px, py, qx, qy) {
...
BAMBOO.math.Scissors.prototype.adj = function(p) {
    var list = [];
    ...
    for ( var y = sy; y <= ey; y++ ) {
        for ( var x = sx; x <= ex; x++ ) {
            if ( x !== p.x || y !== p.y ) {
                list[idx++] = new BAMBOO.math.FastPoint2D(x,y);
            }
        }
    }
...
BAMBOO.math.Scissors.prototype.setPoint = function(sp) {
    ...
    this.pq = new BAMBOO.math.BucketQueue(this.searchGranBits, function(p) {
...
BAMBOO.math.Scissors.prototype.doWork = function() {
    if ( !this.working ) {
        return;
    }
