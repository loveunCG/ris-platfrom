 ...
 */
BAMBOO.image.View = function(image, isSigned)
{
    /**
     ...
     * @type Rescale
     */
    var rescaleLut = new BAMBOO.image.lut.Rescale(
     ...
     * @type Window
     */
    var windowLut = new BAMBOO.image.lut.Window(rescaleLut, isSigned);
     ...
     * @type Object
     */
    var colorMap = BAMBOO.image.lut.plain;
    /**
     * Current position
    ...
    this.setColorMap = function(map) { 
        ...
        if( this.getImage().getPhotometricInterpretation() === "MONOCHROME1") {
            colorMap = BAMBOO.image.lut.invPlain;
        }
	...
	this.getSliceNum = function() {
		return image.getSize().getSliceNum();
	};
 ...
 */
BAMBOO.image.View.prototype.setWindowLevel = function( center, width )
{
 ...
 * Warning: uses the latest set rescale LUT or the default linear one.
 */
BAMBOO.image.View.prototype.setWindowLevelMinMax = function()
{
 ...
 */
BAMBOO.image.View.prototype.goFirstSlice = function()
{
 ...
 * @return {Boolean} False if not in bounds.
 */
BAMBOO.image.View.prototype.goLastSlice = function()
{	
    return this.setCurrentPosition({
        "i": this.getCurrentPosition().i,
        "j": this.getCurrentPosition().j,
        "k": this.getSliceNum()-1
    });
};
/**
 ...
 * @return {Boolean} False if not in bounds.
 */
BAMBOO.image.View.prototype.incrementSliceNb = function()
{
  if (this.getCurrentPosition().k >= this.getSliceNum()-1)
   	{
		return this.setCurrentPosition({
			"i": this.getCurrentPosition().i,
			"j": this.getCurrentPosition().j,
 ...
 * @return {Boolean} False if not in bounds.
 */
BAMBOO.image.View.prototype.decrementSliceNb = function()
{
    return this.setCurrentPosition({
        "i": this.getCurrentPosition().i,
        "j": this.getCurrentPosition().j,
 ...
 * @method clone
 * @return {View} A full copy of this {BAMBOO.image.Image}.
 */
BAMBOO.image.View.prototype.clone = function()
{
    var copy = new BAMBOO.image.View(this.getImage());
 ...
 */
BAMBOO.image.View.prototype.generateImageData = function( array )
{        
	...
	case "YBR_FULL_422":
        ...
        sliceSize = image.getSize().getSliceSize();
        sliceOffset = (sliceNumber || 0) * 4 * sliceSize;
 ...
 */
BAMBOO.image.View.prototype.addEventListener = function(type, listener)
{
 ...
 * @param {Object} listener The method associated with the provided event type.
 */
BAMBOO.image.View.prototype.removeEventListener = function(type, listener)
{
    var listeners = this.getListeners();
    if( !listeners[type] ) {
        return;
 ...
 */
BAMBOO.image.View.prototype.fireEvent = function(event)
{
    var listeners = this.getListeners();
