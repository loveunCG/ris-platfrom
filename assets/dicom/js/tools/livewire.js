 ...
 */
BAMBOO.tool.Livewire = function(app)
{
    /**
     ...
     * @type Path
     */
    var currentPath = new BAMBOO.math.Path();
    /**
     ...
     */
    function clearPaths() {
        path = new BAMBOO.math.Path();
        currentPath = new BAMBOO.math.Path();
    }
     ...
     * @type Scissors
     */
    var scissors = new BAMBOO.math.Scissors();
    ...
    this.mousedown = function(event){
        ...
        if( !self.started ) {
            ...
            var p = new BAMBOO.math.FastPoint2D(event._x, event._y);
            scissors.doTraining(p);
            // add the initial point to the path
            var p0 = new BAMBOO.math.Point2D(event._x, event._y);
            path.addPoint(p0);
            ...
            // anchor point
                ...
                path = currentPath;
                clearParentPoints();
                var pn = new BAMBOO.math.FastPoint2D(event._x, event._y);
    ...
    this.mousemove = function(event){
        ...
        stop = false;
        while (p && !stop) {
            currentPath.addPoint(new BAMBOO.math.Point2D(p.x, p.y));
            if(!parentPoints[p.y]) { 
                stop = true;
            }
            else { 
        ...
        // create shape
        activeShape = new BAMBOO.tool.RoiCreator(currentPath.pointArray, self.style);
        // add shape to group
        shapeGroup.add(activeShape);
        // draw shape command
        command = new BAMBOO.tool.DrawShapeCommand(activeShape, "livewire", app);
        // draw
        command.execute();
    };
    ...
    this.display = function(bool){
        BAMBOO.gui.displayLivewireHtml(bool);
        // TODO why twice?
        this.init();
    };
    ...
    this.init = function()
    {
        ...
        // init html
        //BAMBOO.gui.initLivewireHtml();
        
        //scissors = new BAMBOO.math.Scissors();
 ...
 */
BAMBOO.tool.Livewire.prototype.getHelp = function()
{
    return {
        'title': "Livewire",
 ...
 */
BAMBOO.tool.Livewire.prototype.setLineColour = function(colour)
{
    // set style var
