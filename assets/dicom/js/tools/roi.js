 ...
 */ 
BAMBOO.tool.RoiCreator = function (points, style)
{
    // physical shape
    var roi = new BAMBOO.math.ROI();
    ...
    /*if ( isFinal ) {
        ...
        if ( size > 0 ) {
            ...
            for ( var j = 1; j < size; ++j ) {
                var line = new BAMBOO.math.Line( last, points[j] );
                if( line.getLength() > 2 ) {
                    clean.push( points[j] );
                    last = points[j];
                }
 ...
 */ 
BAMBOO.tool.UpdateRoi = function (roi, anchor)
{
    // parent group
