 ...
 */
BAMBOO.tool.ZoomAndPan = function(app)
{
    /**
    ...
    this.twotouchdown = function(event){
        ...
        self.y0 = event._y;
        // first line
        var point0 = new BAMBOO.math.Point2D(event._x, event._y);
        var point1 = new BAMBOO.math.Point2D(event._x1, event._y1);
        self.line0 = new BAMBOO.math.Line(point0, point1);
    ...
    this.mousemove = function(event){
		...
		if (self.onlypan)
		{
			if (event.button == 0 || event.button == 2) // left button)
			{
				// apply translation
		...
		else
		{
			...
			{
				var step = Math.sqrt(tx*tx+ty*ty) / 100;
				if (tx < 0)
				{
					step *= -1;
				}
				zoomLayers(step, self.x1, self.y1, self.x1, self.y1);
			}
		}
        // reset origin point
        self.x0 = event._xs;
    ...
    this.twotouchmove = function(event){
        ...
        var point0 = new BAMBOO.math.Point2D(event._x, event._y);
        var point1 = new BAMBOO.math.Point2D(event._x1, event._y1);
        var newLine = new BAMBOO.math.Line(point0, point1);
    ...
    this.display = function(bool){
        BAMBOO.gui.displayZoomAndPanHtml(bool);
    };
 ...
 */
BAMBOO.tool.ZoomAndPan.prototype.getHelp = function()
{
    return {
        'title': "Zoom/Pan",
 ...
 * @method init
 */
BAMBOO.tool.ZoomAndPan.prototype.init = function() {
    // nothing to do.
};