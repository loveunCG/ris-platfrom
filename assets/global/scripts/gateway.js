...
medview.gateway.DicomGateway = function () {
  ...
  this.gwScript = "http://localhost:5005/getDicom";
  ...
  */
  ...
  this.parseXmlCFind = function (xml) {
    ...
    console.log('this is my gwInstance', this);
    //  xmlCFind = xml;
...
medview.gateway.DicomGateway.prototype.parseXmlStudy = function (xml) {
  ...
  $(xml).find("response").sort(function (a, b) {
    ...
    liData += '';
    // console.log('this is series ID ===================>', seriesData)
    $('<dd class="series" respNumber="' + respNumber + '" seriesUID="' + seriesUID + '"></dd>').html(liData).appendTo(seriesList);
  });
    ...
    for (var col = 0; col < ig.cols; col++) {
      ...
      if (instance = series.getInstanceNum(instanceNum)) {
        ...
        if (instance.loaded) {
          ...
          console.log(urlDicom);
          instance.load(urlDicom, iv, function (dicomInstance) {
            ig.instanceLoaded(dicomInstance);
          });
        }
      }
