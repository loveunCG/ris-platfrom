CKEDITOR.dialog.add("scaytDialog", function(f) {
    var g = f.scayt,
        k = '<p><img src="' + g.getLogo() + '" /></p><p>' + g.getLocal("version") + g.getVersion() + "</p><p>" + g.getLocal("text_copyrights") + "</p>",
        l = CKEDITOR.document,
        i = {
            isChanged: function() {
                            ...
                            children: [{
                                ...
                                onClick: function() {
                                    var a = this.getDialog(),
                                        e = j,
                                        b = f.scayt,
                                        d = a.getContentElement("dictionaries", "dictionaryName").getValue();
                            ...
                            children: [{
                                ...
                                onClick: function() {
                                    var a = this.getDialog(),
                                        e = f.scayt,
                                        b = j,
                                        d = a.getContentElement("dictionaries", "dictionaryName"),
                                        c = d.getValue();
                                ...
                                onClick: function() {
                                    var a = this.getDialog(),
                                        e = f.scayt,
                                        b = a.getContentElement("dictionaries", "dictionaryName").getValue();
    ...
    f.on("scaytUserDictionaryAction", function(a) {
        var e = a.data.dialog,
            b = e.getContentElement("dictionaries", "dictionaryNote").getElement(),
            d = a.editor.scayt,
            c;
        void 0 === a.data.error ? (c = d.getLocal("message_success_" +
            a.data.command + "Dic"), c = c.replace("%s", a.data.name), b.setText(c), SCAYT.$(b.$).css({
            color: "blue"
        })) : ("" === a.data.name ? b.setText(d.getLocal("message_info_emptyDic")) : (c = d.getLocal("message_error_" + a.data.command + "Dic"), c = c.replace("%s", a.data.name), b.setText(c)), SCAYT.$(b.$).css({
            color: "red"
        }), null != d.getUserDictionaryName() && "" != d.getUserDictionaryName() ? e.getContentElement("dictionaries", "dictionaryName").setValue(d.getUserDictionaryName()) : e.getContentElement("dictionaries", "dictionaryName").setValue(""))
    });
    ...
    var j = {
        ...
        onShow: function() {
            ...
            if (0 != f.config.scayt_uiTabs[2]) {
                var a = f.scayt,
                    ...
                    b = this.getContentElement("dictionaries", "existDic").getElement().getParent(),
        ...
        toggleDictionaryButtons: function(a) {
            var e = this.getContentElement("dictionaries", "existDic").getElement().getParent(),
                b = this.getContentElement("dictionaries", "notExistDic").getElement().getParent();
            a ? (e.show(), b.hide()) : (e.hide(), b.show())
        },
        getChangedOption: function() {
            var a = {};
            if (1 == f.config.scayt_uiTabs[0])
                for (var e = this.getContentElement("options",
        ...
        buildRadioInputs: function(a, e) {
            ...
            var d = "scaytLang_" + e,
                c = CKEDITOR.dom.element.createFromHtml('<input id="' + d + '" type="radio"  value="' + e + '" name="scayt_lang" />'),
